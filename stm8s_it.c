...
uint8_t buffer[45];   // ilk gelen bilgi
uint8_t word[45];     // buffer dan gelen bilgiler word atiliyor islemek için.
uint8_t counter = 0;
uint8_t FunC = 0;
...
extern void ReadHoldingRegisters(void);
extern void WriteSingleRegisters(void);
...
//extern void ADC_Ort(uint8_t response);
extern void ADC_Output(void);
...
INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
{
       ...
       if (TIM2_GetCounter() > T1_5)
 {
  ...
  if( counter == 7){
    ...
    if(word[0] == Address || word[0] == 254)
    {
      
      TIM2_SetCounter(0);
     while (TIM2_GetCounter() < T3_5)
      {
      }
      ...
      if(word[1] == 3)   // Func dogru ise,
      {
        //ADC_Output(); // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        
        ReadHoldingRegisters();
      }
      if(word[1] == 6)   // Func dogru ise,
      {
